...
namespace seal
{
    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
         ...
         @param[in] encrypted The ciphertext to add
         @param[in] plain The plaintext value to add
         @param[in] plain_coeff_count The degree of a plaintext value
         */
        void add_plain_inplace_bfv(Ciphertext &encrypted, const uint64_t &plain, const size_t plain_coeff_count);
        ...
        /**
         ...
         @param[in] plain_coeff_count The degree of a plaintext value
         */
        void add_plain_inplace_ckks(Ciphertext &encrypted, const uint64_t &plain, const size_t plain_coeff_count);
        ...
        /**
         ...
         current encryption parameters, and cannot be identially 0. Dynamic memory
         allocations in the process are allocated from the memory pool pointed to by
         the given MemoryPoolHandle. A ciphertext is a single-precision number,
         so we apply the fast Barrett reduction technique.
         
         @param[in] encrypted The ciphertext to multiply
         @param[in] plain The plaintext to multiply
         @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
         */
        void multiply_plain_inplace_bfv_fast(Ciphertext &encrypted, const Plaintext &plain,
                                             MemoryPoolHandle pool = MemoryManager::GetPool());
        ...
        /**
         ...
         @param[in] encrypted The ciphertext to multiply
         @param[in] plain The plaintext value to multiply
         @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
         */
        void multiply_scalar_inplace_bfv_fast(Ciphertext &encrypted, const std::uint64_t &plain,
                                             MemoryPoolHandle pool = MemoryManager::GetPool());
        ...
        /**
         ...
         @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
         */
        void multiply_plain_inplace_ckks_fast(Ciphertext &encrypted, const Plaintext &plain,
                                              MemoryPoolHandle pool = MemoryManager::GetPool());
        ...
        /**
         ...
         @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
         */
        void multiply_scalar_inplace_ckks_fast(Ciphertext &encrypted, const std::uint64_t &plain,
                                              MemoryPoolHandle pool = MemoryManager::GetPool());
         ...
         */
        void multiply_scalar_ckks_fast(const Ciphertext &encrypted, const std::uint64_t &plain,
                                       Ciphertext &destination,
                                       MemoryPoolHandle pool = MemoryManager::GetPool());
        
        // CHANGED
        /**
         Multiplies a ciphertext with a polynomial X^{-exponenet} = -X^{N-exponent}
         
         @param[in] encrypted The ciphertext to multiply
         @param[in] exponent The exponent value to multiply
         @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
         */
        void negacyclic_shift_poly_inplace(Ciphertext &encrypted, const size_t &exponent,
                                           MemoryPoolHandle pool = MemoryManager::GetPool());
    ...
    private:
        ...
        // CHANGED
        void multiply_plain_normal_bfv_fast(Ciphertext &encrypted, const Plaintext &plain,
                                    util::MemoryPool &pool);
        
        // CHANGED
        void multiply_scalar_normal_bfv_fast(Ciphertext &encrypted, const std::uint64_t &plain,
                                            util::MemoryPool &pool);
        ...
        void multiply_plain_normal_ckks(Ciphertext &encrypted, const Plaintext &plain,
                                   util::MemoryPool &pool);
        // CHANGED
        void multiply_plain_normal_ckks_fast(Ciphertext &encrypted, const Plaintext &plain,
                                    util::MemoryPool &pool);
        
        // CHANGED
        void multiply_scalar_normal_ckks_fast(Ciphertext &encrypted, const std::uint64_t &plain,
                                             util::MemoryPool &pool);
        
        // CHANGED
        void multiply_scalar_normal_ckks_fast(const Ciphertext &encrypted, const std::uint64_t &plain,
                                              Ciphertext &destination, util::MemoryPool &pool);
        
        void negacyclic_shift_poly_normal(Ciphertext &encrypted, const size_t &exponent,
                                          util::MemoryPool &pool);
        // CHANGED
        void add_plain(
        ...
        // CHANAGED
        void add_plain(
            const std::uint64_t &plain, const size_t plain_coeff_count,
            const SEALContext::ContextData &context_data,
            uint64_t *destination) const;
