...
namespace seal
{
    ...
    // CHANGED
    void Evaluator::add_plain(
                              const uint64_t &plain, const size_t plain_coeff_count,
                              const SEALContext::ContextData &context_data,
                              uint64_t *destination) const
    {
        auto &parms = context_data.parms();
        ...
        for (size_t i = 0; i < plain_coeff_count; i++, destination++)
        {
            destination[0] = util::add_uint_uint_mod(destination[0], plain, coeff_modulus[0]);
           
        }
    }
    
    // CHANGED
    void Evaluator::add_plain_inplace_bfv(Ciphertext &encrypted, const uint64_t &plain, const size_t plain_coeff_count)
    {
        ...
        size_t coeff_count = parms.poly_modulus_degree();
        size_t coeff_mod_count = coeff_modulus.size();
        multiply_add_plain_with_scaling_variant(plain, plain_coeff_count, context_data, encrypted.data());
    }
    ...
    // CHANGED
    void Evaluator::add_plain_inplace_ckks(Ciphertext &encrypted, const uint64_t &plain, const size_t plain_coeff_count)
    {
        auto &context_data = *context_->get_context_data(encrypted.parms_id());
        auto &parms = context_data.parms();
        ...
        add_plain(plain, plain_coeff_count, context_data, encrypted.data());
    }
    
    // CHANGED
    void Evaluator::multiply_plain_inplace_bfv_fast(Ciphertext &encrypted,
                                                    const Plaintext &plain, MemoryPoolHandle pool)
    {
        multiply_plain_normal_bfv_fast(encrypted, plain, move(pool));
    }
    
    // plain.scale() = 1, plain.coeff_count()=1
    // coeff_mod_count = 1
    void Evaluator::multiply_plain_normal_bfv_fast(Ciphertext &encrypted,
                                                   const Plaintext &plain, MemoryPool &pool)
    {
        auto &context_data = *context_->get_context_data(encrypted.parms_id());
        auto &parms = context_data.parms();
        ...
        // Multiplying by a monomial?
        size_t mono_exponent = plain.significant_coeff_count() - 1; // mono_exponent = 0
        
        if (plain[mono_exponent] >= plain_upper_half_threshold) {   // plain >= t/2 = 2^9
            for (size_t i = 0; i < encrypted_size; i++) {
                negacyclic_multiply_poly_mono_coeffmod_fast(encrypted.data(i), coeff_count,
                                                            plain[mono_exponent] + plain_upper_half_increment[0],
                                                            mono_exponent, coeff_modulus[0],
                                                            encrypted.data(i), pool);
            }
        }
        else {
            for (size_t i = 0; i < encrypted_size; i++){
                negacyclic_multiply_poly_mono_coeffmod_fast(encrypted.data(i), coeff_count,
                                                            plain[mono_exponent], mono_exponent, coeff_modulus[0],
                                                            encrypted.data(i), pool);
            }
        }
        return;
    ...
    // CHANGED: multiply by a scalar
    void Evaluator::multiply_scalar_inplace_bfv_fast(Ciphertext &encrypted,
                                                    const std::uint64_t &plain, MemoryPoolHandle pool)
    {
        multiply_scalar_normal_bfv_fast(encrypted, plain, move(pool));
    }
    
    void Evaluator::multiply_scalar_normal_bfv_fast(Ciphertext &encrypted,
                                                   const std::uint64_t &plain, MemoryPool &pool)
    {
        auto &context_data = *context_->get_context_data(encrypted.parms_id());
        auto &parms = context_data.parms();
        auto &coeff_modulus = parms.coeff_modulus();
        ...
        if (plain >= plain_upper_half_threshold) {   // plain >= t/2 = 2^9
            for (size_t i = 0; i < encrypted_size; i++) {
                negacyclic_multiply_poly_mono_coeffmod_fast(encrypted.data(i), coeff_count,
                                                            plain + plain_upper_half_increment[0],
                                                            0, coeff_modulus[0],
                                                            encrypted.data(i), pool);
            }
        }
    ...
    void Evaluator::multiply_plain_inplace_ckks_fast(Ciphertext &encrypted,
                                                     const Plaintext &plain, MemoryPoolHandle pool)
    {
        // Verify parameters.
        if (!is_metadata_valid_for(encrypted, context_) || !is_buffer_valid(encrypted))
        ...
        if (encrypted.is_ntt_form())
        {
            throw invalid_argument("this operation does not be supported in NTT form");
        }
        else
        {
            multiply_plain_normal_ckks_fast(encrypted, plain, move(pool));
        }
#ifdef SEAL_THROW_ON_TRANSPARENT_CIPHERTEXT
        // Transparent ciphertext output is not allowed.
        if (encrypted.is_transparent())
    ...
    // CHANGED
    void Evaluator::multiply_plain_normal_ckks_fast(Ciphertext &encrypted,
                                                    const Plaintext &plain, MemoryPool &pool)
    {
        // Extract encryption parameters.
        auto &context_data = *context_->get_context_data(encrypted.parms_id());
        ...
        for (size_t i = 0; i < encrypted_size; i++)
        {
            for (size_t j = 0; j < coeff_mod_count; j++)
            {
                negacyclic_multiply_poly_mono_coeffmod_fast(
                                                            encrypted.data(i) + (j * coeff_count), coeff_count,
                                                            plain[0], 0, coeff_modulus[j],
                                                            encrypted.data(i) + (j * coeff_count), pool);
            }
        }
        return;
    ...
    // CHANGED
    void Evaluator::multiply_scalar_inplace_ckks_fast(Ciphertext &encrypted,
                                                      const uint64_t &plain, MemoryPoolHandle pool)
    {
        multiply_scalar_normal_ckks_fast(encrypted, plain, move(pool));
        
//        //Extract encryption parameters.
//        auto &context_data = *context_->get_context_data(encrypted.parms_id());
//        auto &parms = context_data.parms();
//        auto &coeff_modulus = parms.coeff_modulus();
//        size_t coeff_count = parms.poly_modulus_degree(); // (1 << 10)
//        size_t encrypted_size = encrypted.size();// 2
//        util::MemoryPool &newpool = move(pool);
//
//        for (size_t i = 0; i < encrypted_size; i++){
//            negacyclic_multiply_poly_mono_coeffmod_fast(encrypted.data(i), coeff_count,
//                                                        plain, 0, coeff_modulus[0],
//                                                        encrypted.data(i), newpool);
//        }
        
    }
    
    // CHANGED
    void Evaluator::multiply_scalar_ckks_fast(const Ciphertext &encrypted, const std::uint64_t &plain,
                                              Ciphertext &destination,
                                              MemoryPoolHandle pool)
    {
        multiply_scalar_normal_ckks_fast(encrypted, plain, destination, move(pool));
    }
    
    // CHANGED
    void Evaluator::negacyclic_shift_poly_inplace(Ciphertext &encrypted,
                                        const size_t &exponent, MemoryPoolHandle pool)
    {
        negacyclic_shift_poly_normal(encrypted, exponent, move(pool));
    }
    
    // CHANGED
    void Evaluator::multiply_scalar_normal_ckks_fast(Ciphertext &encrypted,
                                                    const uint64_t &plain, util::MemoryPool &pool)
    {
        //Extract encryption parameters.
        auto &context_data = *context_->get_context_data(encrypted.parms_id());
        auto &parms = context_data.parms();
    ...
    void Evaluator::multiply_scalar_normal_ckks_fast(const Ciphertext &encrypted, const std::uint64_t &plain,
                                                     Ciphertext &destination, util::MemoryPool &pool)
    {
        // Extract encryption parameters.
        auto &context_data = *context_->get_context_data(encrypted.parms_id());
        auto &parms = context_data.parms();
        ...
        for (size_t i = 0; i < encrypted_size; i++){
            negacyclic_multiply_poly_mono_coeffmod_fast(destination.data(i), coeff_count,
                                                        plain, 0, coeff_modulus[0],
                                                        destination.data(i), pool);
        }
        return;
    }
    
    // suppose that exponent < 0
    void Evaluator::negacyclic_shift_poly_normal(Ciphertext &encrypted,
                                                 const size_t &exponent, util::MemoryPool &pool)
    {
        //Extract encryption parameters.
        auto &context_data = *context_->get_context_data(encrypted.parms_id());
        auto &parms = context_data.parms();
        ...
        for (size_t i = 0; i < encrypted_size; i++){
            negacyclic_shift_poly_fast(encrypted.data(i), coeff_count,
                                       exponent, coeff_modulus[0], encrypted.data(i), pool);
        }
