...
namespace seal
{
    ...
    // CHANGED: is_asymmetric = true, save_see = false, 
    void Encryptor::encrypt_zero_internal_fast(
                                          parms_id_type parms_id,
                                          bool is_asymmetric, bool save_seed,
                                          Ciphertext &destination,
                                          MemoryPoolHandle pool) const
    {
        ...
        if (is_asymmetric)
        {
            auto prev_context_data_ptr = context_data.prev_context_data();
            // Does not require modulus switching
            util::encrypt_zero_asymmetric_fast(public_key_, context_, parms_id,
                                          is_ntt_form, destination);
        }
    }
         ...
         uint64_t *destination) const
    {
        ...
        auto &coeff_modulus = parms.coeff_modulus();
        size_t coeff_mod_count = coeff_modulus.size();  // 1
        size_t plain_coeff_count = plain.size();
        
        for (size_t i = 0; i < plain_coeff_count; i++, destination++)
        {
            uint64_t plain_coeff = plain[i];
            for (size_t j = 0; j < coeff_mod_count; j++)
            {
                destination[j * coeff_count] = util::add_uint_uint_mod(
                                                                       destination[j * coeff_count], plain_coeff, coeff_modulus[j]);
    ...
    // CHANGED: bfv
    void Encryptor::encrypt_internal_bfv(
         const Plaintext &plain,
         bool is_asymmetric, bool save_seed,
         Ciphertext &destination,
         MemoryPoolHandle pool) const
    {
        ...
        encrypt_zero_internal_fast(context_->first_parms_id(),
                              is_asymmetric, save_seed, destination, pool);
    ...
    // CHANGED
    void Encryptor::encrypt_internal_ckks(
                                          const vector<int> &plain, long nshift,
                                          bool is_asymmetric, bool save_seed,
                                          Ciphertext &destination,
                                          MemoryPoolHandle pool) const
    {
        ...
        // Enc(0) + (m, 0)
        // first we shift the input data
        vector<uint64_t> uplain (plain.size());
        for(long i = 0; i < plain.size(); ++i){
            //if(plain[i] != 0){
                uplain[i] = static_cast<uint64_t> (plain[i] << nshift);
            //}
        }
        
        add_plain(uplain, *context_->first_context_data(), destination.data());
    }
}
