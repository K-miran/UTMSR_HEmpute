...
namespace seal
{
    namespace util
    {
        ...
        // CHANGED
        void multiply_poly_scalar_coeffmod_fast(const std::uint64_t *poly,
            std::size_t coeff_count, std::uint64_t scalar, const SmallModulus &modulus,
            std::uint64_t *result);
        ...
        // CHANGED
        void negacyclic_shift_poly(const std::uint64_t *operand,
            std::size_t coeff_count, std::size_t shift, const SmallModulus &modulus,
            std::uint64_t *result);
        ...
        // CHANGED
        inline void negacyclic_multiply_poly_mono_coeffmod_fast(
            const std::uint64_t *operand, std::size_t coeff_count,
            std::uint64_t mono_coeff, std::size_t mono_exponent,
            const SmallModulus &modulus, std::uint64_t *result, MemoryPool &pool)
        {
            auto temp(util::allocate_uint(coeff_count, pool));
            multiply_poly_scalar_coeffmod_fast(
                operand, coeff_count, mono_coeff, modulus, temp.get());
        ...
        inline void copy_poly(const std::uint64_t *poly,
        ...
        // CHANGED: X^-k = - X^{coeff - k}
        inline void negacyclic_shift_poly_fast(const std::uint64_t *operand, std::size_t coeff_count,
            size_t exponent, const SmallModulus &modulus, std::uint64_t *result, MemoryPool &pool)
        {
//            auto temp(util::allocate_uint(coeff_count, pool));
//            negacyclic_shift_poly_coeffmod(
//                 operand, coeff_count, coeff_count - exponent,
//                 modulus, temp.get());
//
//            negate_poly_coeffmod(temp.get(),
//                                 coeff_count, modulus, result);
            
            auto temp(util::allocate_uint(coeff_count, pool));
            negacyclic_shift_poly(operand, coeff_count, exponent,
                                  modulus, temp.get());
            
            copy_poly(temp.get(), coeff_count, result);
        }
