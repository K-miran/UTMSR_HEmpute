...
namespace seal
{
    namespace util
    {
        ...
        // CHANGED
        // needed for scalar mult with 32-bit modulus in coefficient representation
        void multiply_poly_scalar_coeffmod_fast(const uint64_t *poly,
            size_t coeff_count, uint64_t scalar, const SmallModulus &modulus,
            uint64_t *result)
        {
            const uint64_t modulus_value = modulus.value();             // the ciphertext modulus q
            const uint64_t const_ratio_1 = modulus.const_ratio()[1];
            for (; coeff_count--; poly++, result++)
            {
        ...
        // CHANGED: X^{-shift} = - X^{n-shift}
        void negacyclic_shift_poly(const uint64_t *operand,
             size_t coeff_count, size_t shift, const SmallModulus &modulus, uint64_t *result)
        {
            // Nothing to do
            if (shift == 0)
            {
...
#endif
#if 1
            size_t new_shift = coeff_count - shift;
            uint64_t index_raw = new_shift;
            uint64_t coeff_count_mod_mask = static_cast<uint64_t>(coeff_count) - 1;
            for (size_t i = 0; i < coeff_count; i++, operand++, index_raw++)
            {
                uint64_t index = index_raw & coeff_count_mod_mask;
