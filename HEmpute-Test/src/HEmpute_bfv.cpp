 ...
 */
void bfvHEmpute::encrypt_data(vector<Ciphertext>& encXData,
                              vector<vector<int>> tag_geno_data, vector<long> tag_model_starting_index, long dim)
{
    ...
    for(long j = first; j < last; ++j){
        encryptor.encrypt_bfv(tag_geno_data[j], encXData[j], MemoryPoolHandle().New(false));
    }
    MT_EXEC_RANGE_END;
}
...
*/
void bfvHEmpute::HEimpute(vector<Ciphertext>& res,
                          vector<Ciphertext> encXData, dvec model0, dmat model,
                          long n_test, double W0scale, double Wscale, size_t plaintext_modulus, vector<long> tag_index)
{
    ...
    MT_EXEC_RANGE(nsnp, first, last);
    for(long j = first; j < last; ++j){
        // encXdata[j_tag],encXdata[j_tag+1], ..., encXdata[j_tag+dim-1],
        ...
        uint64_t coeff;
        while(1){
            if(scale_and_modt(coeff, model[j][k_start], Wscale, plaintext_modulus)){
                break;
            }
            k_start++;
        }
        res[j] = encXData[j_tag + k_start];
        evaluator.multiply_scalar_inplace_bfv_fast(res[j], coeff, MemoryPoolHandle().New(false));
        
        // add the remaining terms
        long k = k_start + 1;
        while(k < dim){
            if(scale_and_modt(coeff, model[j][k], Wscale, plaintext_modulus)){
                Ciphertext ctemp = encXData[j_tag + k];
                evaluator.multiply_scalar_inplace_bfv_fast(ctemp, coeff, MemoryPoolHandle().New(false));
        ...
        if(scale_and_modt(coeff, model0[j], W0scale, plaintext_modulus)){
            evaluator.add_plain_inplace_bfv(res[j], coeff, n_test);
        }
    }
    MT_EXEC_RANGE_END;
}
...
/*
 ...
 @param[in] plaintext_modulus, the plaintext modulus in the bfv scheme
 ...
 */
void bfvHEmpute::decode_Vector(vector<double>& msg, vector<uint64_t> umsg, size_t plaintext_modulus, double scale){
    long n = umsg.size();
    size_t half_plaintext_modulus = (plaintext_modulus + 1) >> 1;
    
    for(long i = 0; i < n; ++i){
        if(umsg[i] < half_plaintext_modulus){
            double tmp = static_cast<double> (umsg[i]) / scale;
            msg.push_back(tmp);
        }
        else{
            uint64_t utmp = plaintext_modulus - umsg[i];
 ...
 */
void bfvHEmpute::decrypt_impute(dmat& res, vector<Ciphertext> encres, size_t plaintext_modulus, double scale)
{
    long nsnp = res.size();
    ...
    for(long j = first; j < last; ++j){
        Plaintext plaintmp;
        decryptor.decrypt(encres[j], plaintmp);
        ...
        vector<double> dmsg;
        decode_Vector(dmsg, umsg, plaintext_modulus, scale);
        res[j] = dmsg;
    }
    MT_EXEC_RANGE_END;
}
...
/*
 ...
 @param[out] encXData[p'], the encryptions of the tag dataset
 */
void bfvHEmpute::encrypt_parallel(vector<Ciphertext>& encXData,
                                  vector<vector<int>> tag_geno_data, vector<long> tag_model_starting_index, long dim, long nparallel)
{
    long length = tag_model_starting_index.size();
    long nsnp = (tag_model_starting_index[length - 1] + dim);   // the last starting one plus dimension (the actual number of tag genotypes for imputation)
    ...
    for(long j = first; j < last; ++j){
        ...
        encryptor.encrypt_bfv(merged_tag_geno_data, encXData[j], MemoryPoolHandle().New(false));
    }
    MT_EXEC_RANGE_END;
    ...
    encryptor.encrypt_bfv(merged_tag_geno_data, encXData[nctxts - 1], MemoryPoolHandle().New(false));
}
 ...
 */
void bfvHEmpute::HEimpute(vector<Ciphertext>& res, vector<bool>& encres_index,
                          ...
                          double W0scale, double Wscale, size_t plaintext_modulus, vector<long> tag_index)
{
    ...
    for(long j = first; j < last; ++j){
        ...
        uint64_t coeff;
        while(1){
            if(scale_and_modt(coeff, model[j][k_start], Wscale, plaintext_modulus)){    // find the first nonzero weight
                break;
            }
            k_start++;
            if(k_start == dim)
        ...
        if(k_start != dim) {
            ...
            // add the (nonzero) intercepts
            if(scale_and_modt(coeff, model0[j], W0scale, plaintext_modulus)){
                evaluator.add_plain_inplace_bfv(res[j], coeff, n_test); // res[j] += sum_{0<=i<ntest} coeff * x^i
            }
        }
    }
    MT_EXEC_RANGE_END;
...
/*
 ...
 Each entry is 1 if at least one scalar multiplication is performed between tagSNP and params; otherwise 0.
 */
void bfvHEmpute::HEimpute_parallel(vector<Ciphertext>& res, vector<bool>& encres_index,
                                   ...
                                   double W0scale, double Wscale, size_t plaintext_modulus, vector<long> tag_index)
{
    ...
    for(long j = first; j < last; ++j){
        ...
        if(k_start != dim)
        {
            ...
            evaluator.multiply_scalar_inplace_bfv_fast(res_chunk[l], coeff, MemoryPoolHandle().New(false));
            encres_index[j] = true;
            indicator[l] = true;
...
/*
 ...
 @param[in] plaintext_modulus, the plaintext modulus in the bfv
 @param[in] scale, the scale of the messages
 @param[out] res, the decrypted results
 */
void bfvHEmpute::decrypt_impute(dmat& res,
