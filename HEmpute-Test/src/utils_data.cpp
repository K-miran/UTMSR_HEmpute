...
/*
 ...
 @param[in] model_path, the file name for trained weights
 @param[in] coordinates_path, the file name for starting or ending coordinates of selected target variants
                 ...
                 vector<string> &target_model_coordinates, string model_path, string coordinates_path)
{
    ...
    long nrows = 0;
    ifstream openFile(model_path.data());
    ...
    SimpleDataFromFile(target_model_coordinates, coordinates_path);
    long n_snptarget_model = target_model_coordinates.size();   // the actual number of target SNPs (80,882)
    
    // 3.2. find the starting/ending coordinates
...
*/
void Read_Params(vector<vector<double>> &model_wt, vector<vector<string>> &tag_model_coordinates,
                 vector<string> &target_model_coordinates, string model_path)
{
    // 1. Read the data and store it as a string-type matrix
    vector<vector<string> > res;
    char split_char = 0x09;
    ...
    // 3.1. read the selected target_geno_model_coordinates
    target_model_coordinates.resize(nrows);
    for(long i = 0; i < nrows; ++i){
        target_model_coordinates[i] = (res[i][num_params2]);
    }
    
    long n_snptarget_model = nrows;   // the actual number of target SNPs (116400)
    ...
    // store the corresponding information of model weights and coordinates
    for(long i = 0; i < n_snptarget_model; ++i){
        for(long d = 0; d < dim1; ++d){
            model_wt[i][d] = atof(res[i][d].c_str());
        }
        for(long d = 0; d < dim; ++d){
            tag_model_coordinates[i][d] = res[i][d + dim1];
        }
        //if(i == n_snptarget_model-1) cout << i << ": " << model_wt[i][0] << "," <<  tag_model_coordinates[i][0] << "," << endl;
        //cout << res[j][dim + dim1] << endl;
    }
}
...
/*
 ...
 @param[in] tag_path, the filename for tag genotype dataset
 @param[in] target_path, the filename for target genotype dataset
 @param[in] selection, 1 when "num of target snps = 20K"; 2 when "num of target snps = 40K"; 3 or 4 when "num of target snps" is the size of the whole SNP variants.
 ...
 @param[out] tag_model_starting_index[n_snptag], starting index of tag variant to be used (0 <= x < 16184)
 @param[out] DATAparam, class of parameters for data
 */
void Read_Genotype(dmat& ytest, dvec& model0, dmat& model,
                   ...
                   string tag_path, string target_path, int selection, bool acc_test)
{
    ...
    switch (selection) {
        ...
        case 3:
            ...
            break;
        case 4:
            n_snptarget_imputed = model_data.size();    //  116,406 (low MAF)
            break;
        case 5:
            n_snptarget_imputed = model_data.size();    // 196,834 (whole)
            break;
    }
    ...
    TagDataFromFile(tag_data_coordinates, tag_str_data, tag_path);
    ...
    if(selection < 5){
        TargetDataFromFile(target_data_coordinates, target_rawdata, target_path, acc_test);
    } else{
        target_data_coordinates = target_model_coordinates;
    }
    
    //cout << target_data_coordinates[0] << "," << target_data_coordinates[1] << endl;
